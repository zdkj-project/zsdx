...
/**
 * 
 * ClassName: TrainTeacherServiceImpl
 * Function:  ADD FUNCTION. 
 * Reason:  ADD REASON(可选). 
 * Description: 师资信息(TRAIN_T_TEACHER)实体Service接口实现类.
 * date: 2017-03-07
 *
 * @author  luoyibo 创建文件
 * @version 0.1
...
public class TrainTeacherServiceImpl extends BaseServiceImpl<TrainTeacher> implements TrainTeacherService{
    ...
    @Resource
    public void setTrainTeacherDao(TrainTeacherDao dao) {
        this.dao = dao;
    }
	private static Logger logger = Logger.getLogger(TrainTeacherServiceImpl.class);
	
	@Override
	public QueryResult<TrainTeacher> list(Integer start, Integer limit, String sort, String filter, Boolean isDelete) {
        QueryResult<TrainTeacher> qResult = this.getPaginationQuery(start, limit, sort, filter, isDelete);
		return qResult;
	}
	/**
	 * 根据主键逻辑删除数据
	...
	public TrainTeacher doUpdateEntity(TrainTeacher entity, SysUser currentUser) {
		// 先拿到已持久化的实体
		TrainTeacher saveEntity = this.get(entity.getUuid());
		try {
	...
	public TrainTeacher doAddEntity(TrainTeacher entity, SysUser currentUser) {
		TrainTeacher saveEntity = new TrainTeacher();
		try {
