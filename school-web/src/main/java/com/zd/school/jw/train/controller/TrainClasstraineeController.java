...
public class TrainClasstraineeController extends FrameWorkController<TrainClasstrainee> implements Constant {
     ...
     * @method downloadAuthority
     */
    @RequestMapping("/downloadAuthority")
    public void downloadAuthority(HttpServletRequest request, HttpServletResponse response) throws Exception {
        try {
            String userId = request.getParameter("userId");
            String roomCode = request.getParameter("roomCode");
            String[] userIdArr = userId.split(",");
            String[] roomCodeArr = roomCode.split(",");
            String hql = "from PtTerm where roomId = ( select uuid from BuildRoominfo where roomCode = ? and roomName =?) and isDelete =0";
            String roomName = "";
            Boolean can = true;
            for (int i = 0; i < userIdArr.length; i++) {
                ...
                if (null == ptTerm) {
                    can = false;
                    logger.error("自动下载" + roomCodeArr[i] + "房间权限下载失败,请手动下载权限:");
            ...
            writeJSON(response, jsonBuilder.returnFailureJson("\"下载权限出现异常!请联系管理员! \""));
        }
    }
