...
public class TrainClassController extends FrameWorkController<TrainClass> implements Constant {
    ...
    @Resource
    TrainClassService thisService;

    @Resource
    TrainClasstraineeService classTraineeService;
    ...
    public void doClassUse(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            sendUserId = request.getParameter("sendUserId");
            ...
            for (SysUser sysUser : sendUserList) {
                ...
                if (Base64Util.isBase64(phone)) {
                    phoneArr += Base64Util.decodeData(phone) + ",";
                }
                sendUserName +=sysUser.getXm()+",";
            }
            sendUserName = sendUserName.substring(0,sendUserName.length()-1);
            phoneArr = phoneArr.substring(0, phoneArr.length() - 1);
            ...
            // 5. 创建酒店预订单
            CreateOrderResponse orderResult = thisService.createOrder(trainClass,sendUserName,phoneArr);
            ...
            if (result != -1 && orderResult.getRspCode() == 0) {
                ...
                logger.error(orderResult.getRspMsg());
                String msg = orderResult.getRspMsg().replaceAll("(\\\r\\\n|\\\r|\\\n|\\\n\\\r)", "");
                    writeJSON(response,
                            jsonBuilder.returnFailureJson("\"请求失败，请联系管理员！" + msg + "\""));
            }
        } catch (Exception e) {
    ...
    public void exportCourseCheckExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        sql = "SELECT classScheduleId,xm,classTraineeId,incardTime,outcardTime,attendResult,attendMinute,isleave,remark,traineeNumber FROM TRAIN_V_CHECKRESULT WHERE classId='" + ids + "'" + " "
                + " order by  traineeNumber asc,xm asc";
