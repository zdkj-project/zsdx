...
/**
 * 
 * ClassName: BuildLaboratorydefinController
 * Function: TODO ADD FUNCTION. 
 * Reason: TODO ADD REASON(可选). 
 * Description: 实验室实体Controller.
 * date: 2016-08-23
 *
 * @author  luoyibo 创建文件
 * @version 0.1
 ...
 */
@Controller
@RequestMapping("/BuildLaboratoryDefine")
public class BuildLaboratoryDefineController extends FrameWorkController<BuildLaboratoryDefine> implements Constant {

    @Resource
    BuildLaboratoryDefineService thisService; // service层接口
    @Resource
    ...
    public void list(@ModelAttribute BuildLaboratoryDefine entity, HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        String strData = ""; // 返回给js的数据
        QueryResult<BuildLaboratoryDefine> qr = thisService.getPaginationQuery(super.start(request), super.limit(request),
     ...
     * 
      * @Title: 增加新实体信息至数据库
      * @Description: TODO
      * @param @param BuildLaboratorydefin 实体类
      * @param @param request
      * @param @param response
      * @param @throws IOException    设定参数
      * @return void    返回类型
    ...
    public void doAdd(BuildLaboratoryDefine entity, HttpServletRequest request, HttpServletResponse response)
        ...
        if (ids != null)
            for (int i = 0; i < ids.length; i++) {
                ...
                BuildLaboratoryDefine perEntity = new BuildLaboratoryDefine();
                ...
                info.setRoomType("4");//设置房间类型为教室--1.宿舍，2.办公室，3.教室，4、实验室，5、功能室，9、其它，0、未分配
    ...
    public void doDelete(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (StringUtils.isEmpty(delIds)) {
            ...
            BuildLaboratoryDefine defin = null;
            ...
            if (fs > 0) {
                writeJSON(response, jsonBuilder.returnSuccessJson("'实验室已分配的未删除，未分配的已删除。'"));
            } else {
                writeJSON(response, jsonBuilder.returnFailureJson("'实验室都已分配了，不允许删除。'"));
            }
