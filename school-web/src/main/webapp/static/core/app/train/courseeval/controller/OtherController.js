 ...
 */
Ext.define("core.train.courseeval.controller.OtherController", {
    extend: "Ext.app.ViewController",
    alias: 'controller.courseeval.otherController',
    mixins: {
    ...
    control: {
         ...
         * 导入的保存按钮
         */
        "panel[xtype=courseeval.courseevalimportform] button[ref=formSave]": {
            beforeclick: function (btn) {
                var self = this;
                var dicForm = btn.up('panel[xtype=courseeval.courseevalimportform]');
                var objForm = dicForm.getForm();
                if (objForm.isValid()) {
                    objForm.submit({
                        url: comm.get('baseUrl') + "/Traincourseeval/importData",
                        waitMsg: '正在导入文件...',
         ...
         * 导入的关闭按钮
         */
        "panel[xtype=courseeval.courseevalimportform] button[ref=formClose]": {
            click: function (btn) {
                var win = btn.up('window');
                //关闭窗体
                win.close();
