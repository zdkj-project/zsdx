...
Ext.define("core.train.coursechkresult.controller.OtherController", {
    ...
    control: {
        ...
        'baseformwin[funCode=remark_detail] button[ref=formSave]':{
            beforeclick:function(btn){
                var self=this;
                var win=btn.up("baseformwin")
                var objForm = win.down("baseform[xtype=coursechkresult.remarkform]");
                ...
                if (formObj.isValid()) {
                    ...
                    Ext.Msg.confirm('温馨提示', "您确定要保存备注信息吗？", function (btn2, text) {
                        if (btn2 == "yes") {
                            ...
                            self.asyncAjax({
                                url: comm.get("baseUrl") + "/TrainCourseattend/doRemark",
                                params: params,
                                //回调代码必须写在里面
