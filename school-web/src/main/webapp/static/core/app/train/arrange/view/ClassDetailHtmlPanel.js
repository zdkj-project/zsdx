	...
	items: [{        
        ...
        tpl: new Ext.XTemplate(
            '<div class="trainClass_classInfo">',
                ...
                '<ul>'+
                    ...
                    '<li>安排状态：{isArrange}</li>',
                    '<li>学员人数：{traineeNum}人</li>',
                    '<li>早餐围/人数：{foodBreakfastNum}人</li>',
                    '<li>午餐围/人数：{foodLunchNum}人</li>',
                    '<li>晚餐围/人数：{foodDinnerNum}人</li>',
        ...
        tpl: new Ext.XTemplate(
            '<div class="trainClass_classTraineeInfo">',
                '<div class="trainClass_title">班级学员列表：{classTraineeTitle}</div>',
                '<ul class="trainClass_gridUl">',
                    '<li><span style="width:5%">序号</span><span style="width:10%">姓名</span><span style="width:5%">性别</span><span style="width:10%">民族</span><span style="width:10%">职务</span><span style="width:10%">行政级别</span><span style="width:15%"">移动电话</span><span style="width:15%">所在单位</span><span style="width:10%">学员类别</span><span style="width:10%">学员状态</span></li>',                        
                '{% if (values.classTrainee.length == 0) %}', 
        ...
        tpl: new Ext.XTemplate(
            '<div class="trainClass_classTraineeInfo">',
                '<div class="trainClass_title">班级课程列表：{classCourseTitle}</div>',
                '<ul class="trainClass_gridUl">',
                ...
                '{% if (values.classCourse.length == 0) %}', 
                    '<li style="width:100%;font-size: 13px;font-weight: 400;text-align: center;line-height: 100px;">此班级暂无课程信息...</li>',
                '{% if (values.classCourse.length == 0 ) return  %}',   //reutrun 表示不执行下面的了，在for里面可以使用break、continue
                    '<tpl for="classCourse">',
        ...
        tpl: new Ext.XTemplate(
            '<div class="trainClass_classTraineeInfo">',
                '<div class="trainClass_title">学员就餐信息：{classFoodTitle}</div>',
                ...
                '<ul class="trainClass_gridUl" style="border-top:none;">',
                    ...
                    '<tpl for="rows">',
                        '<li><span style="width:5%">{[xindex]}</span><span style="width:20%">{xm}</span>',
                            ...
                            '<span style="width:10%"><tpl if="isDelete == 0">正常<tpl elseif="isDelete == 1">删除<tpl elseif="isDelete == 2">新增</tpl></span>',
                        '</li>',
                    '</tpl>',           
                    '<div style="clear:both"></div>',
                '</ul>',   
        ...
        tpl: new Ext.XTemplate(
            '<div class="trainClass_classTraineeInfo">',
                '<div class="trainClass_title">学员住宿信息：{classRoomTitle}</div>',
                '<ul class="trainClass_gridUl">',
                ...
                '{% if (values.classRoom.length == 0) %}', 
                    '<li style="width:100%;font-size:13px;font-weight: 400;text-align: center;line-height: 100px;">此班级暂无学员住宿信息...</li>',
                '{% if (values.classRoom.length == 0 ) return  %}',   //reutrun 表示不执行下面的了，在for里面可以使用break、continue
                    '<tpl for="classRoom">',
                        '<li><span style="width:5%">{[xindex]}</span><span style="width:20%">{xm}</span>',
                            '<span style="width:5%"><tpl if="xbm == 1">男<tpl else>女</tpl></span>',
