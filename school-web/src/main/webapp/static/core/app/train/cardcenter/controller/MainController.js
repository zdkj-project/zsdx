    ...
    control: {
        ...
        "basepanel basegrid[xtype=cardcenter.maingrid] button[ref=gridFastSearchBtn]": {
            beforeclick: function (btn) {
                ...
                filter.push({
                    "type": "string",
                    "value": girdSearchText.getValue(),
                    "field": girdSearchText.getName(),
                    "comparison": ""
                });
         ...
         * 门禁权限下载
         */
        "basegrid[xtype=cardcenter.maingrid] button[ref=downloadAuthority]": {
            beforeclick: function (btn) {
                ...
                if (calssRecords.length != 0 && traineeRecords.length != 0) {
                    Ext.Msg.confirm('提示', '是否下载学员门禁权限?', function (btn, text) {
                        if (btn == 'yes') {
                            var loading = new Ext.LoadMask(basePanel, {
                                msg: '正在下载，请稍等...',
                                removeMask: true// 完成后移除
                            ...
                            Ext.each(traineeRecords, function (rec) {
                                var uuid = rec.data.uuid;
                                userId.push(uuid);
                                var roomName = rec.data.roomName;
                                roomCode.push(roomName);
                            });

                            self.asyncAjax({
                                url: comm.get("baseUrl") + "/TrainClasstrainee/downloadAuthority",
                                params: {
                                    userId: userId.join(","),
